...
from memory.database import (
    DatabaseManager, User, ExternalContext, get_db_session
)
...
class WeatherClient:
    ...
    def get_weather_and_air_quality(self, location: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            logger.error(f"Error fetching real weather data for {location}: {e}")
            return self._get_mock_data(location)
    ...
    def _fetch_air_quality(self, lat: float, lon: float) -> Optional[Dict]:
        ...
        try:
            ...
            logger.error(f"Air quality API request failed for {lat}, {lon}: {e}")
            return None
    ...
    def _get_mock_data(self, location: str) -> Dict[str, Any]:
        ...
        base_data = mock_data.get(location, {
            "temperature": 20,
            "condition": "clear",
            "humidity": 70,
            "air_quality_index": 50,
        ...
        result = {
            ...
            "air_quality_index": base_data["air_quality_index"],
            "air_quality": base_data["air_quality"],
            "recommendations": self._get_activity_recommendations(base_data)
        }
        ...
        try:
            for location in locations:
                try:
                    ...
                    logger.error(f"Error updating weather for {location}: {e}")
                    results[location] = False
            ...
            session.rollback()
            logger.error(f"Database error updating weather data: {e}")
        finally:
            session.close()
